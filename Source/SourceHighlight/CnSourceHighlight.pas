...
{$IFNDEF STAND_ALONE}
    ...
    procedure UpdateCustomIdentList;   // 找出自定义标识符
{$ENDIF}
    ...
    {* 也是按 Lines 的方式来，每行一个 CnList，后者容纳 CustomIdent}
...
// 为了独立运行时不引用，自己写个本地的判断指定窗体是否编辑器窗体
...
{$ENDIF}
  ...
  FHighlightCustomIdent := True; // 默认画自定义标识符
...
{$ENDIF}
    if FStructureHighlight or FBlockMatchHighlight or FCurrentTokenHighlight
      ...
      or FHighlightCustomIdent then // 里头顺便做背景匹配高亮
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Token <> nil) and (Rect.Left >= Context.EditorState.GutterWidth) then  // 该位置是我们解析出来的关键字的位置
      begin
        Layer := Token.ItemLayer - 1;
        if FStructureHighlight then
          ColorFg := GetColorFg(Layer)
  ...
  begin
    ...
    begin
      ...
      if (Token <> nil) and (Rect.Left >= Context.EditorState.GutterWidth) then  // 该位置是我们解析出来的流程控制标识符的位置
      begin
        C := Context.Canvas;
        if FFlowStatementBackground <> clNone then
        begin
